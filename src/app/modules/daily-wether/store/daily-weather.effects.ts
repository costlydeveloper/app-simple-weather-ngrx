...
import {DailyWeatherActionType, RequestDailyWeatherAction, RespondDailyWeatherAction} from './daily-weather.actions';

@Injectable()
export class DailyWeatherEffects {
	@Effect()
	requestDailyWeather$: Observable<Action> = this.actions$.pipe(
		ofType<RequestDailyWeatherAction>(DailyWeatherActionType.REQUEST_DAILY_WEATHER),
